    ...
    <div className="md:hidden">
      ...
      >
        <div
          ...
          className={`rounded-t-[2rem] bg-background/95 px-6 pb-32 pt-6 shadow-[0_-20px_60px_rgba(12,12,15,0.25)] backdrop-blur-xl transition-transform duration-500 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] ${
          ...
          <nav className="flex flex-col" aria-label="Mobile">
            {links.map((link, i) => (
              <Link
                ...
                className={`border-b border-line py-4 text-center text-[24px] font-semibold tracking-tight transition-all duration-500 ${
      ...
      {/* Dock */}
      <div className="pointer-events-none fixed inset-x-0 bottom-8 z-[70] flex items-center justify-center px-6">
        <button
          type="button"
          onClick={() => setOpen(!open)}
          aria-expanded={open}
          aria-label={open ? nav.menuClose : nav.menuOpen}
          className="pointer-events-auto flex h-14 items-center gap-2.5 rounded-full border border-white/40 bg-white/60 pl-5 pr-6 text-[15px] font-semibold text-foreground shadow-[0_10px_30px_rgba(12,12,15,0.18)] backdrop-blur-xl transition-transform active:scale-[0.97]"
        >
        ...
        <a
          ...
          aria-label="WhatsApp"
          className="pointer-events-auto absolute right-6 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_rgba(37,211,102,0.35)] transition-transform active:scale-[0.95]"
        >
