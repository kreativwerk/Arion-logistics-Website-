...
/** The orange brand block: white type on brand orange, inverse CTA. */
    ...
    <section className="bg-accent text-white">
      <div className="mx-auto max-w-[1320px] px-6 py-20 md:px-12 md:py-32">
        <div className="max-w-[62ch]">
          ...
          <p data-reveal className="mt-5 text-[16px] leading-relaxed text-white/85 md:text-lg">
            {dict.partner.lead}
          </p>
        </div>
        <div className="mt-12 grid gap-x-10 gap-y-10 border-t border-white/30 pt-10 md:grid-cols-3">
          {dict.partner.points.map((point, i) => (
            <div key={point.title} data-reveal data-reveal-delay={String(i * 0.08)}>
              ...
              <p className="mt-2.5 text-[14px] leading-relaxed text-white/80">{point.text}</p>
            </div>
          ))}
        </div>
        <div data-reveal className="mt-12">
          <MagneticButton href={`/${locale}/partner`} variant="inverse">
            {dict.partner.cta}
          </MagneticButton>
        </div>
      </div>
